...
def inpt_user_name():
   #win.fill((0, 0, 0))
    ...
    showLabel(makeLabel("New higher score", 40, 100, 100))

    wordBox = makeTextBox(20, 80, 100, 0, "Insert the user name: ",)
    showTextBox(wordBox)
    textBoxInput(wordBox)


def get_user():
    usuario = inpt_user_name()
    return usuario
...
# Functions to save e read the USER NAME Higher score
def update_user_name(nscore, nname):
    score = max_score()

    with open('scores.txt', 'w') as w:
...
def draw_window(surface, grid, score=0, last_score = 0, last_name = ''):
    surface.fill((0, 0, 0))
    ...
    """#Last name
    label_Nn = font.render(last_name, 1, (255, 255, 255))
    #display position
    sx = top_left_x - 240
    sy = top_left_y + 200

    surface.blit(label, (sx +10 , sy - 200))
    surface.blit(label_Nn, (sx +45 , sy - 150))"""
...
def main(win):
    ...
    while run:
        ...
        draw_window(win, grid, score, last_score)#terminar last_name view
